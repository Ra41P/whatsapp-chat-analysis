...
//"Borrowed" from https://github.com/GauthierLemoine/gauth-fr-articles-code/blob/master/scala-ngrams-generator/NGramsGenerator.scala
  ...
  def generate(text: String, minSize: Int, maxSize: Int) =
    {
      ...
      for (
        ngramList <- this.generate_sub(text, minSize, maxSize);
        ngram <- ngramList
      ) yield ngram.toList
    }
  ...
  def generate_sub(text: String, minSize: Int, maxSize: Int) =
    {
      ...
      for (t <- this.getTokenizerIterator(text)) yield {
